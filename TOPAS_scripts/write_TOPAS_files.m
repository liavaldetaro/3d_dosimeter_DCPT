...
plan = plan_data;
beam_files = [];
spot_weight_multiplier = 10000;   % scales number of particles in each spot
...
for i = 1:numel(plan.ion_Energy)
    ...
    %% GEOMETRY
    write_geometry_setup(Source_TOPAS, H_dos, D_dos)
...
write_geometry_setup(Source_all, H_dos, D_dos)
...
function write_geometry_setup(Source, H_dos, D_dos)
    global plan
    fprintf(Source, '##############################################\n');
    ...
    fprintf(Source, 'd:Ge/Patient/Rmin                    = 0.0 mm\n');
    fprintf(Source, 'd:Ge/Patient/Rmax                    = %0.2f mm\n', D_dos);
    fprintf(Source, 'd:Ge/Patient/HL                      = %0.2f mm\n', H_dos);
...
function write_scorer_setup(Source, H_dos, D_dos)
    ...
    fprintf(Source, 'i:Ge/ScoringBox/Xbins                = %i \n', round(2*D_dos/0.25));
    fprintf(Source, 'i:Ge/ScoringBox/Ybins                = %i \n', round(2*D_dos/0.25));
    fprintf(Source, 'i:Ge/ScoringBox/Zbins                = %i \n', round(2*H_dos/0.25));
    fprintf(Source, 'b:Ge/ScoringBox/isParallel           = "True"\n');
    
    disp(round(2*D_dos/0.25));
    ...
    fprintf(Source, 's:Sc/LETatPhantom/Quantity           = "ProtonLET"\n');
    ...
    fprintf(Source, 's:Sc/LET/Quantity           = "ProtonLET"\n');
    fprintf(Source, 's:Sc/LET/Component          = "ScoringBox"\n');
    fprintf(Source, 'd:Sc/LET/MaxScoredLET       = 100 MeV/mm/(g/cm3)\n');
    fprintf(Source, 's:Sc/LET/OutputType                 = "DICOM"\n');
    fprintf(Source, 'b:Sc/LET/DICOMOutput32BitsPerPixel  = "F"\n');
